...
# Redirect stdout to a utf-8 writer
...
print(f"5-Fold CV Accuracy: {[round(s,4) for s in scores]}")
...
for expected, email in tests:
    ...
    feats = str(result["top_features"][:3])
    print(f"[{mark}] [{expected:8s}]  score={result['ml_score']:3d}  label={result['ml_label']:<18s}  top_feats={feats}")

print()
print("ALL PASSED" if all_correct else "SOME FAILED")
